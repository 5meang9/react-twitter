...
export default function ProfilePage(){
  ...
  useEffect(()=>{
    if(user){
      ...
      const myPostsQuery = query(postsRef, where('uid', '==', user.uid), orderBy('createdAt', 'desc'));
      const likePostsQuery = query(postsRef, where('likes', 'array-contains', user.uid), orderBy('createdAt', 'desc'));
        ...
        {activeTab === 'my' && (
          <div className="post">
            {myPosts?.length > 0 ? (
              ...
              <div className='post__no-posts'><div className='post__text'>게시글이 없습니다.</div></div>
            )}
          </div>
        )}
        {activeTab === 'like' && (
